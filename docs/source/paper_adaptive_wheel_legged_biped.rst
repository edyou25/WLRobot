...
论文中用于关节力矩映射的两条关键关系式可以写成：

.. math::

   T_1 =
   -\frac{l_1 \sin(\phi_4-\phi_3)\left(T_p \cos(\phi_0-\phi_2)+F L_0 \sin(\phi_0-\phi_2)\right)}
   {L_0 \sin(\phi_3-\phi_2)}

.. math::

   T_2 =
   -\frac{l_4 \sin(\phi_2-\phi_1)\left(T_p \cos(\phi_0-\phi_3)+F L_0 \sin(\phi_0-\phi_3)\right)}
   {L_0 \sin(\phi_3-\phi_2)}

这里 ``T_p`` 表示虚拟腿转动方向上的等效力矩，``F`` 表示沿虚拟腿方向的推力，``L_0`` 表示虚拟腿长度。它们共同把“虚拟腿上的控制量”转换成实际关节电机扭矩 ``T_1``、``T_2``。

在完成机构简化后，论文把系统状态写成：

.. math::

   x = [\theta,\ \dot{\theta},\ x_b,\ \dot{x}_b,\ \phi,\ \dot{\phi}]^T,
   \qquad
   u = [T_p,\ T]^T

其中 ``\theta`` 是虚拟腿偏角，``x_b`` 是机体前后位移，``\phi`` 是机身俯仰角，``T`` 是轮子输出扭矩。再围绕直立平衡点线性化，可得到连续和离散状态空间模型：
...
其核心是先求解离散 Riccati 方程，再得到最优反馈增益：
...
.. math::
   ...
   K = (H^T P H + R)^{-1} H^T P G

考虑虚拟腿长度 ``L_0`` 会变化，论文进一步把不同腿长下得到的增益矩阵做多项式拟合，最终控制律写成：

.. math::

   u = -K(L_0)(x_d - x)

这里 ``x_d`` 是期望状态，``K(L_0)`` 表示随虚拟腿长度变化的反馈增益。也就是说，机器人在蹲起、伸腿或越障过程中，控制器不会固定使用同一组增益，而是会随着腿长变化调整平衡控制强度。
